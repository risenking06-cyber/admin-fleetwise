...
export default function Lands() {
  const { lands } = useData();
  const crud = useCrudOperations<Land>({
    collectionName: 'lands',
    successMessages: {
      create: 'Land added successfully',
      update: 'Land updated successfully',
      delete: 'Land deleted successfully',
    },
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (crud.editingItem) {
      ...
      await crud.handleCreate(formData as Omit<Land, 'id'>);
    }
    ...
    <div>
      <div className="mb-8 flex justify-between items-center">
        ...
        >
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{crud.editingItem ? 'Edit Land' : 'Add New Land'}</DialogTitle>
            </DialogHeader>
                ...
                <td className="py-3 px-4">
                  <div className="flex justify-end gap-2">
                    <Button 
                      ...
                      size="sm" 
                      onClick={() => {
                        crud.openEditDialog(land);
                        setFormData({ name: land.name });
                      }}
                    >
                    ...
                    <Button 
                      variant="destructive" 
                      size="sm" 
                      onClick={() => crud.openDeleteDialog(land.id)}
                    >
